...
"""
# Regular expression to find URLs starting with "https://www.nupco.com/"
pattern = r"https://www\.nupco\.com/[^\"\s]+"

# Find all matches
matches = re.findall(pattern, text)

# Function to find PDF link in the headers of a URL
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for link in soup.find_all('a', href=True):
                print(link)
                if link.get('target') == '_blank' and 'النتائج النهائية' in link.text:
                    if link['href'].endswith('.pdf'):
                        print('HHAHAHH',link['href'])
                        return link['href']
        return None
    except Exception as e:
        print(f"Error accessing {url}: {e}")
        return None
def find_specific_pdf_link1(url):
    try:
        response = requests.get(url)
        if response.status_code == 200:
            soup = BeautifulSoup(response.content, 'html.parser')
            ...
            for link in soup.find_all('a', href=True):
                print(link)
                if link.get('target') == '_blank' and 'بنود المنافسة' in link.text:
                    if link['href'].endswith('.pdf'):
                        print('HHAHAHH',link['href'])
                        return link['href']
        return None
...
df = pd.DataFrame(matches, columns=['Terms'])

# Find the specific PDF links for each URL and store in the DataFrame
# df['PDF Links'] = df['URLs'].apply(find_specific_pdf_link)
df['PDF Links'] = df['Terms'].apply(find_specific_pdf_link1)

# Save to Excel
df.to_excel('nupco_links_with_pdfs.xlsx', index=False)

print(f"Processed {len(matches)} URLs and saved to 'nupco_links_with_pdfs.xlsx'")
